...
internal sealed class TScanner : Object
{
    ...
    {
        ArgumentNullException.ThrowIfNull(nameof(ATextMap));
        TextMap = ATextMap;
        FColumn = 1;
        FRow = 1;
        Position = 1;
...
public sealed class TUdmfLexer : Object
{
    ...
    {
        return Scanner.Finished ? '\0' : Scanner.GetNextChar();
    }
...
public sealed class TUdmfParser : Object
{
    ...
    private TMapLinedef? GetMapLinedef()
    {
        ...
        if ((ValidV1) && (ValidV2))
            return new TMapLinedef(V1, V2, ID, Blocking, TwoSided, SideFront, SideBack, Special, Arg0, Arg1, Arg2, Arg3, Arg4, MonsterUse, RepeatableSpecial, Ignored);
        else
            return null;
    }
    ...
    private TMapSector GetMapSector()
    {
        ...
        return new TMapSector(HeightFloor, HeightCeiling, ID, Special, Ignored);
    }
    private TMapThing? GetMapThing()
    {
        ...
        do
        {
            ...
            if (string.Equals(Token, "Type", StringComparison.OrdinalIgnoreCase))
            {
                ThingType = GetInt32Token();
                ValidThingType = true;
            }
        ...
        if ((ValidX) && (ValidY) && (ValidThingType))
            return new TMapThing(X, Y, Z, ThingType, ID, Special, Arg0, Arg1, Arg2, Arg3, Arg4);
        else
            return null;
    }
