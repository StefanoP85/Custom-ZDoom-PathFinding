    ...
    public partial class FormMain : Form
    {
        ...
        private void AddArchive(string FileName)
        {
            ...
            try
            {
                if (Archive.Load(FileName))
                {
                    ...
                    Archives.Sort(delegate (TArchive A, TArchive B) { return string.Compare(A.FileName, B.FileName); });
                    UpdateArchives();
                }
            }
            catch (Exception Ex)
        ...
        private void ButtonGenerateNavmesh_Click(object Sender, EventArgs E)
        {
            ...
            if (!(DataGridViewArchives.CurrentCell is null) && (!(DataGridViewMaps.CurrentCell is null)))
            {
                ...
                TNavMeshSettings NavMeshSettings = new TNavMeshSettings();
                NavMeshSettings.ActorHeight = ActorHeight;
                NavMeshSettings.ActorRadius = ActorRadius;
